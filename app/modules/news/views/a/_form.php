...
<?php if($this->context->module->settings['enableThumb']) : ?>
    <?php if($model->thumb) : ?>
        ...
        <a href="'<?= Url::to(['/admin/protectednews/a/clear-image', 'id' => $model->news_id]) ?>" class="text-danger confirm-delete" title="<?= Yii::t('news', 'Clear image')?>"><?= Yii::t('news', 'Clear image')?></a>
    <?php endif; ?>
